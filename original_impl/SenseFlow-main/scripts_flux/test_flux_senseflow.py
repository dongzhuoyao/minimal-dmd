...
if __name__ == "__main__":
    seed_everything(42)
    parser = argparse.ArgumentParser(description="Inference script for SenseFlow FLUX model")
    parser.add_argument("--flux_ckpt", type=str, required=True,
                        help="Path to FLUX.1-dev checkpoint directory")
    parser.add_argument("--checkpoint", type=str, required=True,
                        help="Path to trained SenseFlow checkpoint")
    parser.add_argument("--dit_config", type=str,
                        default="exp_flux/flux-wo-guidance-embed/transformer/config.json",
                        help="Path to DIT transformer config file")
    ...
    # Get absolute path for dit_config relative to script directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    dit_config_path = os.path.join(project_root, args.dit_config)
    
    # Load transformer config and create model
    transformer = FluxTransformer2DModel.from_config(dit_config_path)
    
    folder_name = "senseflow_flux"
    ...
    transformer.eval().to(dtype=torch.bfloat16)
    
    # Load base pipeline and replace transformer
